...
function DocumentManager({ 
  ...
  const handleOpenModal = (doc = null) => {
    if (doc) {
      ...
      setFormData({
        ...
        categoria: doc.categoria || formCategories[0]
      });
    } else {
      setEditingDoc(null);
      setFormData({
  ...
  const handleCloseModal = () => {
    ...
    setEditingDoc(null);
    setFormData({ titulo: '', descripcion: '', categoria: formCategories[0] });
    setSelectedFile(null);
  };
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '3rem' }}>Cargando documentos...</div>;
    ...
    <div className="document-manager">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ flex: '1', minWidth: '280px' }}>
          ...
          <p style={{ color: 'var(--on-surface-variant)' }}>{subtitle}</p>
        </div>
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <h3 style={{ margin: 0 }}>{editingDoc ? 'Editar Registro' : 'Nuevo Registro'}</h3>
              ...
              <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '600', fontSize: '0.85rem' }}>Título</label>
                  <input 
                    type="text" 
                    value={formData.titulo} 
                    onChange={e => setFormData({ ...formData, titulo: e.target.value })} 
                    required 
                    placeholder="Ej. Documento 2024"
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '600', fontSize: '0.85rem' }}>Archivo Adjunto</label>
                  <div style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
                  ...
                  <button type="submit" disabled={saving} className="btn-primary" style={{ padding: '10px 24px' }}>
                    {saving ? 'Guardando...' : (editingDoc ? 'Actualizar' : 'Guardar')}
                  </button>
                </div>
              </form>
            </motion.div>
